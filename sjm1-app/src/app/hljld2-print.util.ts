...
import { HljldPageModel, paginateHljld2, formatHljld2PageNo, addTrailingBlankRows2 } from './hljld2-pagination.core';
import { HLJLD_SHEET_CSS } from './hljld2-sheet.styles';
...
<body>
  <div id="print-root" class="print-root"></div>
    ...
    // 最后一页添加空白行 + "以下空白"
    addTrailingBlankRows2(root, ifrDoc);
 ...
 * 返回总页数和每日页数。
 */
export async function printAllHljld2Records({
  vms,
  remarkLines,
}: {
  vms: HljldViewModel[];
...
<body>
  <div id="print-root" class="print-root"></div>
    ...
    for (const vm of vms) {
      ...
      const pageModels = paginateHljld2(printWindow.document, root, vm, remarkLines, totalPages);
...
function renderPageModel(
  ...
  pageNoSpan.textContent = formatHljld2PageNo(pageModel.indexInDay);
