...
export class PiccoRecordComponent implements OnInit, OnDestroy {
 ...
 signatureAt(time?:string):string{if(!time)return'';const target=databaseTimeValue(time);if(!Number.isFinite(target))return'';for(let i=this.signatureRecords.length-1;i>=0;i--){const s=this.signatureRecords[i];if(s.instant<=target&&s.editUser)return this.accountNameMap.get(s.editUser)||'';}return'';}
