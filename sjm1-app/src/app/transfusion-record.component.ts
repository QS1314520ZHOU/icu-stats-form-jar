...
export class TransfusionRecordComponent implements OnInit, OnDestroy {
  ...
  selectedPageNo = 1; selectedPrintPages: number[] = [];
  ...
  private applyRecord(r: TransfusionRecord): void { this.record = { ...r, pages: Array.isArray(r.pages) ? r.pages : [] }; this.pages = this.record.pages.sort((a, b) => a.pageNo - b.pageNo).map((p, i) => this.normalizePage({ ...p, pageNo: i + 1, persisted: true })); if (!this.pages.length) this.pages = [this.emptyPage(1)]; if (this.selectedPageNo > this.pages.length) this.selectedPageNo = this.pages.length; if (this.selectedPageNo < 1) this.selectedPageNo = 1; this.normalizeSelectedPrintPages(this.pages.length); }
  ...
  private showDefaultBlankPage(): void { const p = this.emptyPage(1); this.record = null; this.defaultPlaceholderPageId = p.pageId; this.pages = [p]; this.selectedPageNo = 1; this.selectedPrintPages = []; this.dirtyPageIds.clear(); this.autoSaveState = 'idle'; }
