...
/**
 * ICU 护理记录单 PDF 控制器（独立副本）
 */
@RestController
@RequestMapping("/api/v1/icu/hljld-new")
public class HljldNewPdfController {

    private static final Logger log = LoggerFactory.getLogger(HljldNewPdfController.class);

    private final HljldFlowPdfService flowPdfService;
    private final FormPageIndexService pageIndexService;

    @Autowired
    public HljldNewPdfController(HljldFlowPdfService flowPdfService,
    ...
    public ResponseEntity<Map<String, Object>> getPageIndex(
            ...
            @RequestParam(required = false) String referenceTime) {
        try {
            FormPageIndexService.PageIndexResult result = pageIndexService.getPageInfo(pid, "hljld-new", date, referenceTime);
    ...
    public ResponseEntity<Map<String, String>> recalculatePageIndexes(
            @PathVariable String pid) {
        try {
            pageIndexService.recalculatePageIndexes(pid, "hljld-new");
    ...
    public ResponseEntity<Map<String, Object>> getRecalculateStatus(
            @PathVariable String pid) {
        Map<String, Object> response = pageIndexService.getCalculationStatus(pid, "hljld-new");
        return ResponseEntity.ok(response);
    }
}
