...
import com.smartcare.backend.hljld.HljldPdfRenderPurposeNew;
import com.smartcare.backend.service.FormPageIndexService;
import com.smartcare.backend.service.HljldFlowPdfServiceNew;
...
/**
 * ICU 护理记录单 PDF 控制器（/form/hljldFormPDFNew 专用）
 */
@RestController
@RequestMapping("/api/v1/icu/hljld-new-new")
public class HljldNewPdfControllerNew {

    private static final Logger log = LoggerFactory.getLogger(HljldNewPdfControllerNew.class);

    private final HljldFlowPdfServiceNew flowPdfService;
    private final FormPageIndexService pageIndexService;

    @Autowired
    public HljldNewPdfControllerNew(HljldFlowPdfServiceNew flowPdfService,
    ...
    public ResponseEntity<byte[]> getPdf(
        ...
        try {
            ...
            log.error("生成PDF失败 (New): pid={}, date={}, referenceTime={}, purpose={}", pid, date, referenceTime, purpose, e);
    ...
    public ResponseEntity<byte[]> getAllPdfs(
        ...
        try {
            ...
            log.error("生成全部PDF失败 (New): pid={}, referenceTime={}", pid, referenceTime, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
    ...
    public ResponseEntity<byte[]> getRangePdf(
        ...
        try {
            ...
            log.error("生成范围PDF失败 (New): pid={}, startDate={}, endDate={}, referenceTime={}",
    ...
    public ResponseEntity<Map<String, Object>> getPageIndex(
        ...
        try {
            ...
            log.error("获取页码信息失败 (New): pid={}, date={}, referenceTime={}", pid, date, referenceTime, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
    ...
    /**
     * 验证 referenceTime 格式（ISO-8601）。
     *
     * @param referenceTime 参考时间字符串
     * @return 错误信息（null 表示格式正确）
     */
