...
public class TestRowBuild2 {
    public static void main(String[] args) {
        ...
        // Step 4: Also test what happens when databaseTimeValue gets a Date directly
        double dbTimeFromDate = HljldUtils.databaseTimeValue(startTime);
        System.out.println("\ndatabaseTimeValue(Date): " + dbTimeFromDate);
        if (!Double.isNaN(dbTimeFromDate)) {
            long mk = (long) Math.floor(dbTimeFromDate / 60000);
            System.out.println("minuteKey from databaseTimeValue(Date): " + mk);
        }
    }
}
